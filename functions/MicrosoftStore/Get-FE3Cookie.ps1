function Get-FE3Cookie {
    <#
    .SYNOPSIS
        Gets an authentication cookie from the FE3 delivery endpoint

    .DESCRIPTION
        Sends a SOAP request to the FE3 Windows Update client web service to obtain
        an encrypted authentication cookie used for subsequent package download requests.

    .OUTPUTS
        String containing the encrypted cookie data, or $null on failure
...
'@
        ...
        $response = Invoke-MSHttpRequest -Uri "https://fe3.delivery.mp.microsoft.com/ClientWebService/client.asmx" `
                                         -Method Post `
                                         -Body $getCookieXml `
                                         -ContentType "application/soap+xml; charset=utf-8"
        
        [xml]$doc = $response.Content
        $cookieNode = $doc.GetElementsByTagName("EncryptedData")
        
        if ($cookieNode.Count -gt 0) {
            return $cookieNode[0].InnerText
        }
        
        return $null
    }
    catch {
        Write-Verbose "Get-FE3Cookie error: $_"
        return $null
    }
}
