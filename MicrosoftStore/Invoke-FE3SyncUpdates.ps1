function Invoke-FE3SyncUpdates {
    <#
    .SYNOPSIS
        Synchronizes updates with the FE3 delivery endpoint

    .DESCRIPTION
        Sends a SyncUpdates SOAP request to the FE3 Windows Update client web service
        using the WuCategoryId to retrieve available packages for a Store application.
        The response XML is stored in $script:SyncUpdatesXml for dependency parsing.

    .PARAMETER WuCategoryId
        The Windows Update category GUID from DisplayCatalog fulfillment data

    .PARAMETER MSAToken
        Optional MSA Device Token. Uses module-cached token if not provided.

    .OUTPUTS
        XML string containing the SyncUpdates response
    ...
    try {
        ...
        Write-Verbose "Cookie obtained: $($cookie.Substring(0, [Math]::Min(50, $cookie.Length)))..."
...
'@
        ...
        $soapBody = $wuRequestTemplate -f $cookie, $WuCategoryId, $MSAToken
        
        # Save request for inspection
        $debugPath = Join-Path $env:TEMP "FE3_SyncUpdates_Request.xml"
        $soapBody | Out-File -FilePath $debugPath -Encoding UTF8
        Write-Verbose "SyncUpdates request saved to: $debugPath"
        ...
        # Store the XML for dependency parsing later
        $script:SyncUpdatesXml = $content
        
        # Save response for debugging
        $debugPath = Join-Path $env:TEMP "FE3_SyncUpdates_Response.xml"
        $content | Out-File -FilePath $debugPath -Encoding UTF8
        Write-Verbose "SyncUpdates response saved to: $debugPath"
        
        return $content
    }
    catch {
        Write-Verbose "Invoke-FE3SyncUpdates error: $_"
        throw
    }
}